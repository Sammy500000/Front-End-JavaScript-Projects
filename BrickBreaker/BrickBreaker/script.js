...
let powerUpX = Math.random() * CANVAS_WIDTH;
let powerUpY = Math.random() * (CANVAS_HEIGHT / 2);
let powerUpRadius = 10;
...
const obstacle = {
    ...
    createGrid: function () {
        for (let i = 0; i < OBSTACLE_ROW_COUNT; i++) {
            ...
            for (let j = 0; j < OBSTACLE_COL_COUNT; j++) {
...
function drawParticles() {
    particles.forEach((particle, index) => {
        ...
        particle.alpha -= 0.02;
        particle.color = particle.color.replace(/[\d\.]+\)$/, particle.alpha + ')');
    });
}
    ...
    move: function () {
        ...
        for (let i = 0; i < LOC.length; i++) {
            if (this.x >= LOC[i][0] && this.x <= LOC[i][0] + OBSTACLE_WIDTH && this.y - this.radius >= LOC[i][1] && this.y - this.radius <= LOC[i][1] + OBSTACLE_HEIGHT) {
                ...
                createParticles(LOC[i][0] + OBSTACLE_WIDTH / 2, LOC[i][1] + OBSTACLE_HEIGHT / 2);
                LOC.splice(i, 1);
                this.vy = -this.vy;
                playBreakSound();
                CURRENT_SCORE += 1;
        ...
        if (!powerUpActive && Math.sqrt((this.x - powerUpX) ** 2 + (this.y - powerUpY) ** 2) < this.radius + powerUpRadius) {
            powerUpActive = true;
            PADDLE_WIDTH *= 1.5;
            setTimeout(() => {
                PADDLE_WIDTH /= 1.5;
                powerUpActive = false;
                powerUpX = Math.random() * CANVAS_WIDTH;
                powerUpY = Math.random() * (CANVAS_HEIGHT / 2);
...
function drawPowerUp() {
    if (!powerUpActive) {
        ...
        ctx.arc(powerUpX, powerUpY, powerUpRadius, 0, Math.PI * 2, true);
        ctx.fill();
        ctx.closePath();
    }
}
...
function x() {
    ...
    const countdownInterval = setInterval(() => {
        if (countdown > 0) {
            Text.draw(`Game starts in ${countdown} sec!`);
            countdown--;
        } else {
